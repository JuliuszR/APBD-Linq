...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldReturnEmployeesFromChicago()
    {
        ...
        List<Emp> result = emps.Where(e =>
            Database.GetDepts().Where(d => d.Loc == "CHICAGO").Select(d => d.DeptNo).Contains(e.DeptNo)).ToList();
    ...
    public void ShouldJoinEmployeesWithDepartments()
    {
        ...
        var result = emps.Join(depts, e => e.DeptNo, e => e.DeptNo,
            (emp, dept) => new { DName = dept.DName, EName = emp.EName });
    ...
    public void ShouldReturnEmployeesWithCommission()
    {
        ...
        var result = emps.Where(e => e.Comm.HasValue).Select(e => new { EName = e.EName, Comm = e.Comm });
    ...
    public void ShouldMatchEmployeeToSalaryGrade()
    {
        ...
        var result = emps.SelectMany(e =>
            grades.Where(s => e.Sal >= s.Losal && e.Sal <= s.Hisal).Select(s => new { e.EName, s.Grade }));
    ...
    public void ShouldReturnEmployeesEarningMoreThanDeptAverage()
    {
        ...
        var result = emps.Where(e => e.Sal > emps.Where(inner => inner.DeptNo == e.DeptNo).Average(inner => inner.Sal))
            .Select(e => e.EName);
